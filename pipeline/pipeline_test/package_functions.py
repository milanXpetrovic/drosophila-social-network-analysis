...
def load_dfs_to_list(path, file_format='.csv'):
...
def round_coordinates(df, decimal_places=0):
    ...
    df = df.round({'pos_x': decimal_places, 'pos_y': decimal_places, 'ori': decimal_places})
...
def prepproc(df, min_x, min_y):
    ...
    # df['ori'] = df['ori'].round(decimals=2)
    ...
    for i in range(len(files)):
        ...
        if next_flie <= len(files):     
            for j in range(next_flie, len(files)):   
                ...
                distances_df[name] = np.sqrt(np.square(df['x_axis_dif']) + 
