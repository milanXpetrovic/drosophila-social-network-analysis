...
def get_path_values(df, window_size):
    ...
    df["step"] = np.sqrt(np.square(df["pos_x"].diff()) + np.square(df["pos_y"].diff()))

    df["spl"] = np.sqrt(
        np.square(df["pos_x"].diff(periods=window_size))
...
def add_multiedges_to_undirected_g(
    G, df, DISTANCE_BETWEEN_FLIES, TOUCH_DURATION_FRAMES
):
    for column in df.columns:
        df_ = df[column]
        df_ = df_[df_ <= DISTANCE_BETWEEN_FLIES + 1]
        ...
        if len(clear_list_of_df) >= 1:
            ...
            for interaction in clear_list_of_df:
                G.add_edge(node_1, node_2, duration=len(interaction))

    return G


def dynamics_of_network_graph(G, df, DISTANCE_BETWEEN_FLIES, TOUCH_DURATION_FRAMES):
    for column in df.columns:
        df_ = df[column]
        df_ = df_[df_ <= DISTANCE_BETWEEN_FLIES + 1]
        clear_list_of_df = [
    ...
    while G.number_of_edges() < number_of_edges:
        ...
        if G.has_edge(node_1, node_2):
            ...
            if old_duration + new_duration > 600:
                ...
                G[node_1][node_2]["averaging"] = float(1 / 599.5) * (
                    float(duration / count) - 0.5
                )
...
def create_n_samples_of_random_graph(g, pop_name, n_samples, FPS):
    ...
    df[pop_name.replace(".gml", "") + "_mean_random"] = total.loc[:, graph_cols].mean(
        axis=1
    )
...
def remove_nodes_with_degree_less_than(G, degree):
    remove = [
        node for node, node_degree in dict(G.degree()).items() if node_degree < degree
    ]
...
def network_measures_distribution():
    graph_functions = [
        ...
        (
            ...
            lambda g: nx.information_centrality(
...
def stat_test(d):
    ...
    df_res = pd.DataFrame.from_dict(
        stat_test_results, orient="index", columns=["t_statistic", "p_value"]
    )
